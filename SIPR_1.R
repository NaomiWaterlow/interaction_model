#Estimates sigma and rho 


Arguments<- commandArgs(trailingOnly = T)
Sim_no<-Arguments[1]
Sig_value <- Arguments[2]
Rho_value <- Arguments[3]
Adapt_start <-Arguments[4]
Adapt_stop <- Arguments[5]
Length_run <-Arguments[6]
seed_no <- Arguments[7]
...
############# LOOK AT SIMULATION ################
#
# observed <- melt(observed.data)
# OBS <- ggplot(observed, aes(x=week_begin, y=value, group = variable)) + geom_line(aes(col=variable))
# OBS
#
